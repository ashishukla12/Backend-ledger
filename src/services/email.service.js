...
async function sendTransactionEmail(userEmail, name, amount, toAccount) {
  ...
  await sendEmail(user, subject, text, html);
}

async function sendTransactionFailureEmail(userEmail, name, amount, toAccount){
  const subject = "Transaction Failed!!";
  const text = `Hello ${name}, \n\nYour transaction of $${amount} to account ${toAccount} was failed.\n\nBest regards, \nThe Backend Ledger Team`;
  const html = `<p>Hello ${name},</p><p>Your transaction of $${amount} to account ${toAccount} was failed.</p><p>Best regards,<br>The Backend Ledger Team</p>`;
